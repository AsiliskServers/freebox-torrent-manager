...
/**
 * Endpoint pour récupérer le journal d'un téléchargement
 */
...
export default defineEventHandler(async (event) => {
  ...
  const { downloadId } = query;
  
  if (!downloadId) {
    throw createError({
      statusCode: 400,
      message: 'downloadId requis'
    });
  }
  ...
  try {
    ...
    const response = await $fetch<FreeboxResponse<string>>(
      `${apiUrl}/downloads/${downloadId}/log`,
      {
        headers: {
